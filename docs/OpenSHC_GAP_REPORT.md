...
> **Last updated**: 2026-02-15 — AutoPoser/LegPoser/BodyPoseController auto-pose lifecycle fully rewritten for 1:1 OpenSHC parity. AutoPoser: mutually exclusive gravity, normaliser scaling, PosingState lifecycle. LegPoser::updateAutoPose: global auto-pose + per-leg negation with smoothStep. BPC: walk-state→PosingState coordination, gait-synced phase calculation, state-dependent `poseForLegManipulation`, `swing_progress_scaler`. Shared enums (`PosingState`, `PoseResetMode`, `SequenceSelection`) unified in `gait_types.h`. Equivalence analysis document deleted. 29/30 tests pass (1 pre-existing failure: `tripod_walk_visualization_test`).
> **Packed/unpacked parity pass (2026-02-13)**: `BodyPoseController::packLegs()`/`unpackLegs()` identified as dead code, `executePackSequence()`/`executeUnpackSequence()` use instant `setRobotJointAngles()` instead of OpenSHC's smooth Bezier via `transitionConfiguration()`. Report updated with detailed comparison table, recommended fix, and `JOINT_TOLERANCE` unit ambiguity analysis.
...
| `PosingState`        | `POSING`, `STOP_POSING`, `POSING_COMPLETE`                                                                 | `PosingState` in `gait_types.h`                       | `POSE_POSING`, `POSE_STOP_POSING`, `POSE_POSING_COMPLETE`                                                                                  | ✅ 1:1 (renamed). Moved to `gait_types.h` (shared header) to avoid duplication.                                                                                                         |
| `PoseResetMode`      | `NO_RESET`, `Z_AND_YAW_RESET`, `X_AND_Y_RESET`, `PITCH_AND_ROLL_RESET`, `ALL_RESET`, `IMMEDIATE_ALL_RESET` | `PoseResetMode` in `gait_types.h`                     | `POSE_RESET_NONE`, `POSE_RESET_Z_AND_YAW`, `POSE_RESET_X_AND_Y`, `POSE_RESET_PITCH_AND_ROLL`, `POSE_RESET_ALL`, `POSE_RESET_IMMEDIATE_ALL` | ✅ 1:1 (renamed). Moved to `gait_types.h` (shared header) to avoid duplication.                                                                                                         |
...
| `SequenceSelection`  | `START_UP`, `SHUT_DOWN`                                                                                    | `SequenceSelection` in `gait_types.h`                 | `START_UP`, `SHUT_DOWN`                                                                                                                    | ✅ 1:1. Moved to `gait_types.h` (shared header) to avoid duplication. Names now match OpenSHC exactly.                                                                                  |
...
| `JOINT_TOLERANCE` (0.01)           | `JOINT_TOLERANCE` (0.1f)                | `state_controller.h` — **unit ambiguity**: 0.1 rad ≈ 5.7° (10× looser than OpenSHC) or 0.1° ≈ 0.0017 rad (tighter). See Priority 11. |
...
| `setAutoPoseParams()`                | `setAutoPoseParams()`                                               | ✅ (gait-synced phase calculation with normaliser/reference leg).                                            |
| `updateStance()`                     | `applyAutoPoseToDesiredTips()`                                      | ✅ (renamed; applies per-leg auto-pose to desired tips).                                                     |
...
| `poseForLegManipulation()`           | `poseForLegManipulation()`                                          | ✅ (state-dependent: LEG_WALKING_TO_MANUAL gets inclination+step_height; others get current-manual+default). |
| `packLegs(time)`                     | `packLegs(Leg[], time)`                                             | **Dead code** (see Priority 11).                                                                             |
| `unpackLegs(time)`                   | `unpackLegs(Leg[], time)`                                           | **Dead code** (see Priority 11).                                                                             |
...
**OpenSHC `AutoPoser` class** → Ported 1:1 as `AutoPoser` in `auto_poser.h`:

- `AutoPoserContainer` with multiple `AutoPoser` objects → HexaMotion `AutoPoser` class with X/Y/Z amplitude components and gravity amplitude.
- `AutoPoser::updatePose(phase, phase_length, normaliser, posing_state, pose_frequency, gravity)` — 1:1 with OpenSHC: mutually exclusive gravity/xyz amplitudes, normaliser-scaled start/end phases, PosingState-aware lifecycle with `start_check_`/`end_check_` pair logic, iteration-based `delta_t = 1.0 / (num_iterations / 2.0)`, quartic Bézier curve generation.
- `BodyPoseController::updateAutoPose()` — walk-state→PosingState coordination (WALK*STARTING/MOVING→POSING, zero_velocity+WALK_STOPPING/STOPPED→STOP_POSING), sync_with_step_cycle when `pose_frequency* == -1.0`.
- `BodyPoseController::setAutoPoseParams()` — gait-synced phase calculation: `base_phase_length = stance_phase + swing_phase`, `raw_phase_length = ((1/step_freq)/time_delta) / swing_ratio`, per-leg negation params and reference leg detection from `offset_multiplier`.
- `LegPoser::updateAutoPose(phase, global_auto_pose, normaliser, phase_length)` — takes global auto-pose from BPC, applies per-leg negation via iteration/first_half/smoothStep logic matching OpenSHC exactly.
- **Note**: `LegPoser::updateAutoPose` omits OpenSHC's `step_state != FORCE_STANCE` check because HexaMotion's `LegPoser` has no `LegStepper` access. Walk state coordination is handled externally via BPC setters.
...
| `updateAutoPose(phase)`                                                                             | `updateAutoPose(phase, global_auto_pose, normaliser, phase_length)`                                     | ✅ (takes global auto-pose from BPC, applies per-leg negation with smoothStep). |
...
- `setPoseNegationPhaseStart()` / `setPoseNegationPhaseEnd()` / `setNegationTransitionRatio()` — per-leg auto-pose negation parameters set by BPC.
...
| 13  | **Smooth Bezier pack/unpack transition** | `PoseController::packLegs()`/`unpackLegs()` call `LegPoser::transitionConfiguration()` every tick for cubic Bezier interpolation over `PACK_TIME / step_frequency` seconds                 | **Resolved**        | `executePackSequence()`/`executeUnpackSequence()` now use `LegPoser::transitionConfiguration()` with cubic Bezier. See Priority 11. |
| 14  | **Multi-step packed configurations**     | Per-joint `packed_positions_` vector: `packed`, `packed_0`, `packed_1`... for sequential fold stages, reversed on unpack                                                                   | **Not implemented** | Single packed target per leg. See Priority 11.                                                                                      |
...
**Resolved items (2026-02-14)**: Pack/unpack transitions now use smooth cubic Bezier via `LegPoser::transitionConfiguration()` (1:1 with OpenSHC). `JOINT_TOLERANCE` fixed to 0.01 radians. Default packed/unpacked poses match OpenSHC YAML defaults. Dead code `BodyPoseController::packLegs()`/`unpackLegs()` removed. See Priority 11 section.

**Resolved items (2026-02-15)**: AutoPoser/LegPoser/BodyPoseController auto-pose lifecycle fully rewritten for 1:1 OpenSHC parity. AutoPoser: mutually exclusive gravity, normaliser scaling, PosingState lifecycle. LegPoser::updateAutoPose: global auto-pose + per-leg negation with smoothStep. BPC: walk-state→PosingState coordination, gait-synced phase calculation, state-dependent `poseForLegManipulation`, `swing_progress_scaler`. Shared enums unified in `gait_types.h`. Equivalence analysis document (`BODYPOSE_CONTROLLER_OPENSHC_EQUIVALENCE_ANALYSIS.md`) deleted. See Priority 12 section.
...
| "AutoPoser system not replicated"                               | **Present** as 1:1 `AutoPoser` class in `auto_poser.h` (mutually exclusive gravity, normaliser scaling, PosingState lifecycle), `LegPoser::updateAutoPose()` (per-leg negation with smoothStep), and `BodyPoseController` auto-pose coordination (gait-synced phase, walk-state→PosingState). `IMUAutoPose` provides separate IMU-based posing.                                                                                                   |
...
| :-------------------------- | :-------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Auto-Pose Appl.**         | Adds `auto_pose` to Body, subtracts from Legs ($Body+Auto$, $Leg-Auto$).    | Global `auto_pose_` computed by `AutoPoser`, per-leg negation applied in `LegPoser::updateAutoPose()` via smoothStep. Body-level auto-pose added in `updateCurrentPose()` when IMU inactive. | **Resolved** (2026-02-15). 1:1 lifecycle: mutually exclusive gravity, normaliser scaling, PosingState coordination, gait-synced phase calculation. Per-leg negation omits FORCE_STANCE check (no LegStepper access in LegPoser). |
...
| **Pack/Unpack Smoothness**  | Smooth cubic Bezier transition over `PACK_TIME / step_frequency` (~2 s).    | Smooth cubic Bezier via `LegPoser::transitionConfiguration()` over `PACK_TIME / step_frequency` seconds (1:1 with OpenSHC).                                                                  | **Resolved**. Pack/unpack now uses per-leg Bezier interpolation identical to OpenSHC.                                                                                                                                            |
| **`JOINT_TOLERANCE` Value** | 0.01 radians (~0.57°) in `pose_controller.h`.                               | 0.01 radians in `state_controller.h` (matched to OpenSHC).                                                                                                                                   | **Resolved**. Units documented as radians; value matches OpenSHC exactly.                                                                                                                                                        |
...
### Priority 11: Packed/Unpacked Configuration Parity (2026-02-13, updated 2026-02-14)

HexaMotion now supports explicit packed/unpacked joint targets through Parameters and uses them in the state machine for transitions and state detection. Smooth Bezier pack/unpack transitions match OpenSHC 1:1.

- [x] **Configured packed/unpacked poses in Parameters**: Added per-leg pose targets and enable flag.
    - New fields in [src/robot_model.h](src/robot_model.h):
        - `use_configured_packed_unpacked_poses` (default: `true`)
        - `packed_pose_joints[NUM_LEGS]` (default: OpenSHC YAML values: coxa=-1.571, femur=1.900, tibia=1.200 rad)
        - `unpacked_pose_joints[NUM_LEGS]` (default: OpenSHC YAML values: coxa=0.000, femur=0.785, tibia=-1.138 rad)
    - Units are radians (robot-model convention), matching OpenSHC joint model units.
    - `Parameters` constructor initializes defaults matching OpenSHC `default.yaml`.

- [x] **Smooth Bezier pack/unpack transitions (1:1 with OpenSHC)**:
    - [src/state_controller.cpp](src/state_controller.cpp):
        - `executePackSequence()` and `executeUnpackSequence()` now call `LegPoser::setDesiredConfiguration()` (once at start) then `LegPoser::transitionConfiguration(PACK_TIME / step_frequency)` each tick for all legs.
        - Uses `executing_pack_transition_` flag to prevent re-setting desired configuration mid-Bezier (mirrors OpenSHC `executing_transition_`).
        - Returns minimum progress across all legs (not instant `PROGRESS_COMPLETE`).
        - This matches OpenSHC's `PoseController::packLegs()` → `LegPoser::transitionConfiguration()` flow exactly.
    - [src/state_controller.h](src/state_controller.h):
        - `JOINT_TOLERANCE` fixed to `0.01` radians (matching OpenSHC's `pose_controller.h` value exactly).
        - Added `executing_pack_transition_` member for multi-tick Bezier tracking.

- [x] **Dead code removed**:
    - `BodyPoseController::packLegs()` and `unpackLegs()` declarations and implementations removed from [src/body_pose_controller.h](src/body_pose_controller.h) and [src/body_pose_controller.cpp](src/body_pose_controller.cpp).
    - Associated `pack_step_` member and its initialization/reset in `BodyPoseController` also removed.
    - These were dead code (never called) with hardcoded Cartesian positions that did not match OpenSHC's joint-space Bezier approach.

- [x] **State machine uses configured targets for pack/unpack**:
    - [src/state_controller.cpp](src/state_controller.cpp):
        - `initialize(...)` loads packed/ready target joint angles from Parameters when enabled.
        - `isRobotPacked()` and `isRobotReady()` use pure joint-target comparison when configured mode is enabled.

- [x] **Context and hardware pipeline integration**:
    - Added context API `setRobotJointAngles(...)` in [src/state_controller_context.h](src/state_controller_context.h).
    - Implemented in [src/locomotion_system.h](src/locomotion_system.h) and [src/locomotion_system.cpp](src/locomotion_system.cpp):
        - Sends joint commands to servos for all legs.
        - Updates leg joint/tip state and body pose estimate.

- [x] **Test stub integration**:
    - Added default packed/unpacked test configuration helper in [tests/test_stubs.h](tests/test_stubs.h).
    - Added alias header [tests/test_stub.h](tests/test_stub.h) and updated key integration tests to include it.

#### OpenSHC vs HexaMotion — Detailed Comparison for packed/unpacked

| Topic                                                | OpenSHC                                                                                                                                              | HexaMotion (current)                                                                                                                                              | Status                                                                 |
| :--------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------- |
| Configuration source                                 | YAML joint parameters (`packed`, `unpacked`, optional `packed_1..N`)                                                                                 | `Parameters` struct fields with OpenSHC-matching defaults. Constructor sets packed/unpacked to OpenSHC YAML values                                                | **Equivalent behavior, different transport** (by design: no YAML/ROS)  |
| Multi-step packed chains                             | Supports multiple packed stages via `packed_positions_` vector per joint                                                                             | Single packed pose target per leg in Parameters                                                                                                                   | **Partial parity** — multi-stage fold choreography not yet represented |
| Smooth Bezier transition during pack/unpack          | `packLegs()`/`unpackLegs()` call `LegPoser::transitionConfiguration()` every tick for smooth cubic Bezier over `PACK_TIME / step_frequency` seconds  | `executePackSequence()`/`executeUnpackSequence()` call `LegPoser::transitionConfiguration()` every tick for smooth cubic Bezier over `PACK_TIME / step_frequency` | **1:1** — identical Bezier interpolation path                          |
| `executing_transition_` flag                         | Prevents re-setting desired configuration mid-Bezier in `PoseController`                                                                             | `executing_pack_transition_` in `StateController` prevents re-setting targets mid-Bezier                                                                          | **1:1** — same guard semantics                                         |
| `BodyPoseController::packLegs()`/`unpackLegs()`      | Called by `StateController::transitionRobotState()` as the primary pack/unpack path                                                                  | Removed (was dead code). Pack/unpack logic lives in `StateController::executePackSequence()`/`executeUnpackSequence()` using `LegPoser` directly                  | **Equivalent** — same outcome, different organizational location       |
| `JOINT_TOLERANCE` for state detection                | 0.01 radians (≈0.57°) defined in `pose_controller.h`                                                                                                 | 0.01 radians defined in `state_controller.h` with explicit documentation                                                                                          | **1:1** — matched exactly                                              |
| UNKNOWN state detection                              | Joint tolerance against configured packed/unpacked positions via per-joint iteration                                                                 | Same logic when configured mode enabled; legacy body-position heuristic fallback when disabled                                                                    | **Equivalent in configured mode**                                      |
| Pack/unpack execution call path                      | `StateController::transitionRobotState()` → `PoseController::packLegs(PACK_TIME/freq)` every tick → per-joint Bezier via `transitionConfiguration()` | `StateController::handleRobotStateTransition()` → `executePackSequence()` → per-leg `LegPoser::transitionConfiguration(PACK_TIME/freq)` every tick                | **1:1** — same multi-tick Bezier execution model                       |
| Direct startup semantics (`start_up_sequence=false`) | OpenSHC direct flow still anchored in packed/ready model states                                                                                      | Preserved; configured unpacked targets are used when enabled                                                                                                      | **Equivalent**                                                         |
| Default packed joint values                          | YAML `packed`: coxa=-1.571, femur=1.900, tibia=1.200 (all legs identical)                                                                            | `Parameters()` constructor default: identical values                                                                                                              | **1:1**                                                                |
| Default unpacked joint values                        | YAML `unpacked`: coxa=0.000, femur=0.785, tibia=-1.138 (all legs identical)                                                                          | `Parameters()` constructor default: identical values                                                                                                              | **1:1**                                                                |

#### Validation

- Rebuilt and executed key integration tests after all fixes:
    - `coxa_phase_transition_test` — PASS
    - `pose_gait_integration_test` — PASS (4/4)
    - `tripod_linearity_test` — PASS
    - `virtual_hardware_sim_test` — PASS

### Priority 12: AutoPoser/LegPoser/BodyPoseController Auto-Pose Lifecycle 1:1 Parity (2026-02-15)

Nine gaps identified in the equivalence analysis document (`BODYPOSE_CONTROLLER_OPENSHC_EQUIVALENCE_ANALYSIS.md`) were resolved. The document was deleted after all gaps were implemented. All changes maintain **functional 1:1 parity** with OpenSHC's `PoseController` auto-pose subsystem.

- [x] **GAP 1 — Unified shared enums**: `PosingState`, `PoseResetMode`, and `SequenceSelection` enums were duplicated between `body_pose_controller.h` (nested) and `state_controller.h` (free). Moved all three to `gait_types.h` as the single authoritative location. Updated references in `locomotion_system.cpp` (`BodyPoseController::START_UP` → `START_UP`) and `pose_controller_test.cpp` (`BodyPoseController::ALL_RESET` → `ALL_RESET`). `SequenceSelection` values renamed from `SEQUENCE_START_UP`/`SEQUENCE_SHUT_DOWN` to `START_UP`/`SHUT_DOWN` to match OpenSHC exactly.
    - **Source files**: [src/gait_types.h](src/gait_types.h), [src/body_pose_controller.h](src/body_pose_controller.h), [src/state_controller.h](src/state_controller.h), [src/locomotion_system.cpp](src/locomotion_system.cpp), [tests/pose_controller_test.cpp](tests/pose_controller_test.cpp).

- [x] **GAP 2 — BPC `updateAutoPose()` lifecycle coordination**: Added walk-state → PosingState coordination matching OpenSHC's `PoseController::updateAutoPose()`. When `current_walk_state_` is `WALK_STARTING` or `WALK_MOVING`, PosingState is set to `POSING`. When body velocity is zero and walk state is `WALK_STOPPING` or `WALK_STOPPED`, PosingState is set to `STOP_POSING`. Added sync*with_step_cycle logic: when `pose_frequency* == -1.0`, uses provided master phase from walk controller; otherwise free-running with `pose*phase* = (pose*phase* + 1) % pose*phase_length*`. Updated `AutoPoser::updatePose()`and`LegPoser::updateAutoPose()` calls with new signatures.
    - **Source files**: [src/body_pose_controller.cpp](src/body_pose_controller.cpp), [src/body_pose_controller.h](src/body_pose_controller.h).
    - **New BPC setters**: `setCurrentWalkState(WalkState)`, `setBodyVelocityZero(bool)`.

- [x] **GAP 3 — AutoPoser gravity mutual exclusion**: OpenSHC's AutoPoser uses mutually exclusive amplitude modes: `if (gravity_amplitude_ != 0.0)` uses gravity-based amplitude, else uses xyz amplitude. HexaMotion was previously ADDITIVE (gravity + xyz). Fixed to match OpenSHC's exclusive `if/else` pattern.
    - **Source files**: [src/auto_poser.h](src/auto_poser.h).

- [x] **GAP 4 — AutoPoser normaliser scaling and PosingState lifecycle**: Complete rewrite of `AutoPoser::updatePose()`:
    - Phase start/end now scaled by normaliser: `start_phase_ * normaliser`, `end_phase_ * normaliser`.
    - PosingState-aware lifecycle with `start_check_`/`end_check_` pair logic: `start_check_` triggers at phase == 0 when POSING, `end_check_` is a `std::pair<bool, bool>` that tracks STOP_POSING and phase-end alignment.
    - Uses iteration-based `delta_t = 1.0 / (num_iterations / 2.0)` with offset for 2nd half, matching OpenSHC exactly.
    - New signature: `updatePose(int phase, int phase_length, int normaliser, int posing_state, double pose_frequency, const Eigen::Vector3d &gravity_direction)`.
    - **Source files**: [src/auto_poser.h](src/auto_poser.h).

- [x] **GAP 5 — LegPoser `updateAutoPose()` rewrite**: Complete rewrite matching OpenSHC's per-leg auto-pose negation:
    - New signature: `void updateAutoPose(int phase, const Pose &global_auto_pose, int normaliser, int phase_length)`.
    - Takes global `auto_pose_` from BPC instead of computing per-leg from AutoPoseConfiguration.
    - Applies per-leg negation using iteration-based first_half/smoothStep logic: calculates iteration from `(phase % normaliser) + 1`, determines `first_half = (iteration <= iterations_per_half)`, generates `control_input` from negation phase start/end/transition and smoothStep for smooth transitions.
    - Removed old `computeAxis` averaging + orientation-induced offset algorithm and `base_auto_pose_` member.
    - Added setters: `setPoseNegationPhaseStart()`, `setPoseNegationPhaseEnd()`, `setNegationTransitionRatio()`.
    - **Note**: Omits OpenSHC's `step_state != FORCE_STANCE` check because HexaMotion's `LegPoser` has no `LegStepper` access. Walk state coordination is handled externally via BPC setters.
    - **Source files**: [src/leg_poser.h](src/leg_poser.h), [src/leg_poser.cpp](src/leg_poser.cpp).

- [x] **GAP 6 — BPC `setAutoPoseParams()` gait-synced calculation**: When `pose_frequency_ == -1.0` (gait-synced mode):
    - `base_phase_length = stance_phase + swing_phase`
    - `swing_ratio = swing_phase / base_phase_length`
    - `raw_phase_length = ((1 / step_frequency) / time_delta) / swing_ratio`
    - Per-leg negation params setup via `leg_poser->setPoseNegationPhaseStart/End/TransitionRatio()`.
    - Reference leg detection from `offset_multiplier` map values.
    - **New BPC setter**: `setGaitPhaseParams(int stance_phase, int swing_phase, int phase_offset)` — stores gait params for auto-pose phase calculation.
    - **Source files**: [src/body_pose_controller.cpp](src/body_pose_controller.cpp), [src/body_pose_controller.h](src/body_pose_controller.h).

- [x] **GAP 8 — `poseForLegManipulation()` state-dependent logic**: OpenSHC differentiates leg states in `poseForLegManipulation()`. HexaMotion now matches:
    - `LEG_WALKING_TO_MANUAL`: target tip = `inclination_pose_ * (default_tip + Δ_step_height)`; step height subtracted from z; sets current tip pose and zero step height.
    - Other states: target tip = `(body_pose_current_ - manual_pose_ + default_pose_).inverseTransformVector(default_tip)`. `LEG_MANUAL_TO_WALKING` sets default tip pose.
    - Step height changed from `standing_height * 0.1` to `body_pose_config.swing_height` matching OpenSHC.
    - **Source files**: [src/body_pose_controller.cpp](src/body_pose_controller.cpp).

- [x] **GAP 9 — `swing_progress_scaler` in `updateWalkPlanePose()`**: Added `swing_progress_scaler = max(1.0, swing_phase / phase_offset)` and `swing_progress = legs[i].getSwingProgress() * swing_progress_scaler`, matching OpenSHC's walk plane pose swing progress scaling.
    - **Source files**: [src/body_pose_controller.cpp](src/body_pose_controller.cpp).

#### Remaining Architectural Differences (by design)

| Difference                               | OpenSHC                                                        | HexaMotion                                                                                 | Rationale                                                                                                                                        |
| :--------------------------------------- | :------------------------------------------------------------- | :----------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------- |
| FORCE_STANCE check in auto-pose negation | `LegPoser::updateAutoPose` checks `step_state != FORCE_STANCE` | Omitted — `LegPoser` has no `LegStepper` access                                            | Architectural: LegPoser and LegStepper are separate classes with no cross-reference. Walk state coordination handled externally via BPC setters. |
| Walk state / gait params injection       | `PoseController` has `walker_` back-pointer                    | BPC uses setters: `setCurrentWalkState()`, `setBodyVelocityZero()`, `setGaitPhaseParams()` | Architectural: BPC has no WalkController reference. Caller (LocomotionSystem/StateController) forwards walk state.                               |

#### Validation

- Full test suite rebuilt and executed:
    - 29/30 tests PASS
    - 1 pre-existing failure: `tripod_walk_visualization_test` (unrelated to auto-pose changes)
...
| New gaps discovered                     | 7     | Gaps #12–#16 added (Bezier through-point, stringFormat, acquisition timeout, transitionStance, origin*walk_plane_pose*). Gaps #13–#14 added (smooth Bezier pack/unpack transition, multi-step packed configurations). See Priority 11 detailed audit.                                                                                                                      |
...
| Packed/unpacked audit (2026-02-13)      | 3     | `BodyPoseController::packLegs()`/`unpackLegs()` identified as dead code (never called). `executePackSequence()`/`executeUnpackSequence()` use instant `setRobotJointAngles()` instead of OpenSHC's smooth Bezier via `transitionConfiguration()`. `JOINT_TOLERANCE` unit ambiguity documented. See Priority 11 section.                                                    |
...
| Auto-pose lifecycle parity (2026-02-15) | 9     | AutoPoser/LegPoser/BPC auto-pose lifecycle fully rewritten for 1:1 OpenSHC parity. 9 gaps resolved: AutoPoser gravity exclusion, normaliser scaling, PosingState lifecycle, LegPoser negation, BPC gait-synced phase, enum unification, poseForLegManipulation, swing_progress_scaler. 7 files modified. 29/30 tests pass. See Priority 12 section.                        |
