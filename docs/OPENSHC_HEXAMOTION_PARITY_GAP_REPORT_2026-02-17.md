...
**Note on `adjustParameter()`:** This is newly listed as intentionally excluded per the updated `AGENTS.md`: _"OpenSHC's dynamic parameter adjustment path via `StateController::adjustParameter()` is intentionally not ported to HexaMotion. It is replaced by explicit parameter-specific `LocomotionSystem` setter APIs."_ All 9 OpenSHC adjustable parameter types now have dedicated HexaMotion setter methods with proper validation:
