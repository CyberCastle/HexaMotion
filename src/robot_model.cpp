...
Point3D RobotModel::forwardKinematicsGlobalCoordinatesAnalytic(int leg_index, const JointAngles &angles) const {
...
Eigen::Matrix4d RobotModel::legTransformAnalytic(int leg_index, const JointAngles &q) const {
...
Eigen::Matrix3d RobotModel::calculateJacobianAnalytic(int leg, const JointAngles &q, const Point3D &) const {
...
// Helper method to build DH transforms for a leg
std::vector<Eigen::Matrix4d> RobotModel::buildDHTransformsAnalytic(int leg, const JointAngles &q) const {
    std::vector<Eigen::Matrix4d> transforms(DOF_PER_LEG + 1);
