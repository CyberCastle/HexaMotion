...
- `strict_openshc_parity` (master parity switch in `Parameters`, default **false**) reproduces OpenSHC's `LegStepper` trajectory pipeline verbatim and disables ALL HexaMotion-only stability extensions (stride freezing, hybrid anti-drift, lateral residual cleanup, phase-end snap, in-gait workspace constraining and swing-end Z snapping). It has the highest precedence: when `true`, the fine-grained flags (`preserve_swing_end_pose`, `enable_phase_end_snap`, `enable_workspace_constrain`, `drift_*`) are ignored. By default HexaMotion keeps its stability extensions active.
- IK strategy discrepancy (intentional): HexaMotion runs a bounded internal DLS convergence loop (≤30 iterations, 1 mm early-exit) with an analytic seed and a 5°/iteration clamp, instead of OpenSHC's single DLS step per control cycle. This converges within one ~50 Hz MCU cycle and the analytic seed lowers the average iteration count. The fine-grained timing rounding, manual leg toggle, walk-plane fit and `forceNormalTouchdown` node-separation formula are aligned 1:1 with OpenSHC and are therefore no longer divergences.
...
- No dead, legacy, backward-compatible, deprecated or unused code may be committed. Remove unused parameters, fields, branches and helpers instead of guarding or commenting them out. Every symbol must have at least one live use.
...
- Longer tests usually include: `walk_controller_test`, `virtual_hardware_sim_test`, `tripod_walk_test` (visualization sub-test), and `trajectory_test` (hexapod-analysis sub-test).

### Test coverage matrix

Tests are grouped into a small set of consolidated executables. Each consolidated
target links a thin driver (a `run_*()` dispatcher) with the original per-test
translation units; every original test keeps all of its coverage (its former
`int main()` was renamed to `int run_<name>()` and is invoked by the driver). No
test coverage was removed during consolidation.

#### Consolidated test executables

| Consolidated test          | Sub-tests (run\_\* components)                                                                                                                                         | What it covers                                                                                                        | Main software pieces                                                                                                                |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `numeric_integration_test` | `runge_kutta_validation`, `quaternion_functions`                                                                                                                       | Runge-Kutta integration and quaternion utility behavior                                                               | `admittance_controller`, `math_utils`, full locomotion stack                                                                        |
| `ik_test`                  | `simple_ik`, `simple_advanced_ik`, `ik_tracking_diagnostic`, `ik_internal_loop`                                                                                        | Basic/advanced IK, tracking diagnostics, bounded internal DLS loop (§2.5)                                             | `robot_model`, `leg`, `leg_stepper`, `walk_controller`, `math_utils`, `workspace_analyzer`                                          |
| `dh_kinematics_test`       | `simple_dh`, `dh_vs_analytic`, `complete_physical_offset`, `joint_output_calibration`                                                                                  | DH forward kinematics, DH-vs-analytic equivalence, physical offset, joint calibration                                 | `robot_model`, `analytic_robot_model`, `math_utils`, `workspace_analyzer`, full locomotion stack                                    |
| `bezier_test`              | `bezier_validation`, `bezier_curve_deterministic`, `bezier_transition_single_leg`, `bezier_transition_all_legs`                                                        | Bezier support/workspace checks, deterministic control-node formulas, single/all-leg transitions                      | `leg_stepper`, `walk_controller`, `robot_model`, `math_utils`, `workspace_analyzer`                                                 |
| `coxa_stride_test`         | `coxa_phase_transition`, `coxa_stride_decomposition`, `coxa_tripod_symmetry_analytic`, `swing_coxa_orientation`, `stride_vector_validation`, `stride_deviation_limits` | Coxa phase transitions, stride decomposition, tripod symmetry, swing orientation, stride vectors and deviation limits | `leg_stepper`, `walk_controller`, `gait_config_factory`, `velocity_limits`, `stride_deviation_limits`, full locomotion stack        |
| `trajectory_test`          | `trajectory_tip_position`, `trajectory_all_legs`, `hexapod_trajectory_analysis`                                                                                        | Single-foot and multi-leg trajectories, global hexapod trajectory continuity                                          | `walk_controller`, `leg_stepper`, `robot_model`, `velocity_limits`, `terrain_adaptation`                                            |
| `tripod_walk_test`         | `tripod_linearity`, `tripod_walk_visualization`                                                                                                                        | Tripod gait linearity/path consistency and full-stack progression                                                     | `walk_controller`, `leg_stepper`, `state_controller`, `locomotion_system`, `cartesian_velocity_controller`, `admittance_controller` |
| `runtime_api_test`         | `runtime_parameter_setter`, `convenience_motion_api`, `publication_accessor`                                                                                           | Runtime parameter setters, convenience motion API, publication accessors                                              | `locomotion_system`, `state_controller`, full locomotion stack                                                                      |
...
| `default_configuration_tripod_pose_transition_test` | Default-config tripod pose transition flow            | `walk_controller`, `leg_stepper`, `state_controller`, `locomotion_system`, `cartesian_velocity_controller`, `admittance_controller`                         |
...
| `timing_rounding_parity_test`                       | Swing/stance iteration rounding parity with OpenSHC   | `leg_stepper`, `robot_model`, `math_utils`, `workspace_analyzer`, `gait_config_factory`, `body_pose_controller`, `velocity_limits`, `terrain_adaptation`    |
| `force_normal_touchdown_parity_test`                | `forceNormalTouchdown` node-separation formula parity | `leg_stepper`, `robot_model`, `math_utils`, `workspace_analyzer`, `gait_config_factory`, `body_pose_controller`, `velocity_limits`, `terrain_adaptation`    |
| `strict_parity_legstepper_test`                     | `strict_openshc_parity` swing-target freeze toggle    | `leg_stepper`, `robot_model`, `math_utils`, `workspace_analyzer`, `gait_config_factory`, `body_pose_controller`, `velocity_limits`, `terrain_adaptation`    |
| `rough_terrain_adaptation_test`                     | Rough-terrain swing-target projection/step-depth      | `terrain_adaptation`, `robot_model`, `math_utils`, `workspace_analyzer`                                                                                     |
| `walk_plane_all_legs_test`                          | Walk-plane least-squares fit over all default tips    | `robot_model`, `math_utils`, `workspace_analyzer`                                                                                                           |
| `manual_leg_toggle_dual_test`                       | Dual primary/secondary manual leg toggle parity       | `locomotion_system`, `state_controller`, `walk_controller`, `body_pose_controller`, `leg_poser`, `robot_model`                                              |
